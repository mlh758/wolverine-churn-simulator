...
**Superseded. Nothing here reproduces on current Wolverine.**

Every entry below measures a build from *before* **V6.36.0**, which is where the GH-3987 reconcile
sweep ([#4404](https://github.com/JasperFx/wolverine/pull/4404)) and its GH-4407 hardening shipped.
That sweep is what these runs were built to provoke: the duplicate-agent divergence, the
"assigned but not running" wedge, and the convergence behaviour after a rolling deploy. Wolverine
releases often and the fixes landed, so re-running any of this against a current package measures
a different system and will not reproduce the numbers.

They are kept for provenance only — *why* a check exists, what the pathology looked like, and what
the fix was measured against. For the reasoning that is still load-bearing, see
`docs/harness-traps.md`; for live results, `RESULTS.md`.

The locally-packed pre-6.36 Wolverine builds these entries were taken on (`localfeed/*.nupkg`)
have been deleted, as have the three largest streaming-capture run directories
(`runs/iter3-gate-off`, `runs/iter5-gate-off`, `runs/settle15` — 903 MB between them). Paths
cited below may therefore no longer exist; they are a record of what was measured, not something
to open.
